...
def createShape(prefix=''):
    ...
    cmds.transformLimits(ctrl, tx=(0, 0), ty=(0, 0), tz=(-1, 1), etx=(1, 1), ety=(1, 1), etz=(1, 1))

    ctrlBox = cmds.curve(d=1, p=[(0, 0, -1), (0, 0, 1)], k=[0, 1], n=prefix + '_Path')
    ...
    # create Main CtrlBox
    mainCtrlBox = cmds.curve(d=1, p=[((0 - (textWidth/2 + 0.2)), 0, (1.7 + textHeight)),
                                     ((textWidth/2 + 0.2), 0, (1.7 + textHeight)),
                                     ((textWidth/2 + 0.2), 0, -1.3),
                                     ((0 - (textWidth/2 + 0.2)), 0, -1.3),
                                     ((0 - (textWidth/2 + 0.2)), 0, (1.7 + textHeight))],
